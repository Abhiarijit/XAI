"""
transformer_model.py — TransformerNATOPS classifier with attention capture.
...
Key methods used by TAGFC:
    attention_rollout(x_np) → (T,)  temporal saliency via Abnar & Zuidema (2020)
    input_gradient(x_np, target_class) → (T, D)  ∂logit/∂input
"""
...
class TransformerNATOPS(nn.Module):
    """
    6-class Transformer classifier for NATOPS gesture recognition.
    Supports attention rollout and input-gradient computation for TAGFC.
    """
    ...
    def forward(self, x: torch.Tensor, capture_attn: bool = False) -> torch.Tensor:
        ...
        h      = self.ln_out(h)
        h_pool = h.mean(dim=1)
        return self.head(self.drop(h_pool))

    @torch.no_grad()
    def attention_rollout(self, x_np: np.ndarray) -> np.ndarray:
        """
        Attention Rollout (Abnar & Zuidema 2020).
        x_np : (T, D) → returns normalised saliency (T,)
        """
        self.eval()
    ...
    def predict_proba(self, x_np: np.ndarray) -> np.ndarray:
        """x_np: (T, D) or (N, T, D) → probabilities"""
        self.eval()
        squeeze = x_np.ndim == 2
        if squeeze:
            x_np = x_np[None]
...
def train_transformer(model, train_loader, test_loader,
                      c: Config = cfg, device: str = "cpu") -> dict:
...
def load_model(path, c: Config = cfg) -> TransformerNATOPS:
