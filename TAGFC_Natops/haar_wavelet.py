...
For T=51, levels=3:  L = 64,  n_a = 8
    [ approx(8) | detail_L3(8) | detail_L2(16) | detail_L1(32) ]  total=64
"""
...
if __name__ == "__main__":
    ...
    x     = rng.standard_normal(51).astype(np.float32)
    c     = haar.dwt(x)
    x_hat = haar.idwt(c, original_length=51)
    print(f"1-D round-trip error (T=51): {np.abs(x - x_hat).max():.2e}")
    print(f"Coefficient length L={len(c)}  (next_pow2(51)=64)")

    X     = rng.standard_normal((51, 24)).astype(np.float32)
    C     = haar.dwt2d(X)
    X_hat = haar.idwt2d(C, original_length=51)
    print(f"2-D round-trip error (T=51, D=24): {np.abs(X - X_hat).max():.2e}")
    print(f"Coefficient shape: {C.shape}   (D=24, L={haar.coeff_len(51)})")
