"""
baselines.py — CoMTE and AB-CF counterfactual baselines for AQI_XAI.

CoMTE  — Ates et al., ICAPAI 2021
         Greedy nearest-unlike-neighbour channel swap.

AB-CF  — Li et al., DaWaK 2023
         Entropy-guided segment swap from nearest unlike neighbour.
"""
...
class CoMTE:
    ...
    At each step picks the single untouched pollutant channel whose replacement
...
class ABCF:
    ...
    Scores every (pollutant, time-segment) pair by the Shannon entropy of the
    model output when only that segment is unmasked.  Lowest-entropy pairs
    ...
    def __init__(self, model, X_train: np.ndarray, y_train: np.ndarray,
        ...
        """
        window_frac : segment length as fraction of T (default 15% → ~2 days for T=14)
        """
        self.model       = model
        self.X_train     = X_train
        self.y_train     = y_train
