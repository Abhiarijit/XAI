...
const doc = new Document({
  ...
  sections: [{
    ...
    children: [
      ...
      twoColTable([
        ['Step 1', 'ATTENTION ROLLOUT: Multiply residual-corrected attention matrices A^(l) across all L Transformer layers and H heads. Formula: A_hat^(l) = 0.5*A^(l) + 0.5*I. Rollout = A_hat^1 * A_hat^2 * ... * A_hat^L. Column sums = temporal saliency s_t in [0,1]^T.', LLBLUE],
        ['Step 2', 'HAAR WAVELET TRANSFORM: Apply DWT to each sensor independently. C^d = W_psi(X[:,d]). Produces coefficients across 3 frequency scales: Approximation (slow trend), Detail-2 (medium), Detail-1 (fast noise). Stores C in R^{DxL} (14 sensors x 28 coefficients).', LLBLUE],
      ...
      twoColTable([
        ...
        ['How TAGFC uses it', 'Step 1 of TAGFC. Extracts attention matrices from ALL L layers, ALL H heads. Applies residual correction. Multiplies across layers. Gets s_t in [0,1]^T which guides omega computation.'],
      ]),
      ...
      twoColTable([
        ...
        ['Output', 's in [0,1]^T where T=30 cycles. High s_t = Transformer attended to cycle t'],
      ]),
      space(),
      ...
      twoColTable([
        ...
        ['Full matrix', 'C in R^{D x L} = R^{14 x 28} for FD001. Perturbation delta in R^{D x L}'],
      ...
      twoColTable([
        ...
        ['Real FD001 data', 'data/raw/CMAPSS/ (train_FD001.txt + test_FD001.txt + RUL_FD001.txt)'],
      ]),
      space(),
...
const outPath = path.join(__dirname, 'TAGFC_Session_Summary.docx');
