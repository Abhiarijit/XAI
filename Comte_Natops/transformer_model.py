...
Architecture:
    ...
    N_LAYERS × TransformerEncoderLayer  [multi-head self-attention + FFN]
    Global average pool → (B, d_model)
    Linear → (B, K)

Attention weights from all heads/layers are stored for attention rollout
(used by TAGFC; not needed for CoMTE but kept for model compatibility).
"""
...
# ─────────────────────────────────────────────────────────────────────────────
# Transformer layer that stores attention weights
# ─────────────────────────────────────────────────────────────────────────────
...
class AttentionLayer(nn.Module):
    def __init__(self, d_model: int, n_heads: int, d_ff: int, dropout: float):
        ...
        self.dropout = nn.Dropout(dropout)
        self.attn_weights: torch.Tensor | None = None   # stored after each forward

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        attn_out, w = self.attn(x, x, x, need_weights=True, average_attn_weights=True)
        self.attn_weights = w.detach()                  # (B, T, T)
...
class TransformerNATOPS(nn.Module):
    ...
    def predict_proba(self, X: np.ndarray) -> np.ndarray:
        """X: (N, T, D) numpy → probabilities (N, K)"""
        self.eval()
        with torch.no_grad():
    ...
    def predict(self, X: np.ndarray) -> int:
        """X: (T, D) numpy → predicted class int"""
        return int(self.predict_proba(X[None])[0].argmax())

    def get_attention_weights(self) -> list[np.ndarray]:
        """Return stored attention weights from last forward pass, per layer."""
        return [layer.attn_weights.cpu().numpy() for layer in self.layers
...
def evaluate(model: TransformerNATOPS,
             loader: DataLoader,
             criterion: nn.Module,
...
def train_transformer(model: TransformerNATOPS,
                      train_dl: DataLoader,
                      test_dl:  DataLoader,
                      c: Config = cfg,
                      device: str = "cpu") -> dict:
...
def save_model(model: TransformerNATOPS, path: Path) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    torch.save(model.state_dict(), path)
    print(f"  Model saved → {path}")


def load_model(path: Path, c: Config = cfg) -> TransformerNATOPS:
