"""
main.py — Full TAGFC pipeline on AQI India (city_day.csv).

Steps
-----
1. Load & preprocess AQI India data
2. Train (or load) TransformerAQI
...
from data_pipeline import load_aqi, get_dataloaders
from transformer_model import TransformerAQI, train_transformer, save_model, load_model, evaluate
...
def _target_class(y_orig: int, K: int = 3) -> int:
    """Default target: flip Poor→Good, else flip to Poor."""
...
def step1_data(c: Config = cfg):
    _banner("Step 1 — Load & Preprocess AQI India Data")
    X_tr, y_tr, X_te, y_te, stats = load_aqi(c)
...
def step2_transformer(train_dl, test_dl, c: Config = cfg):
    _banner("Step 2 — Train TransformerAQI")

    model_path = c.MODEL_DIR / "transformer_aqi.pt"
...
def main(c: Config = cfg):
    ...
    results_path = c.OUTPUT_DIR / "results_aqi.pkl"
