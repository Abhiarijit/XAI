...
Metrics (per X_orig / X_cf pair):
    ...
    rcf              — relative CF distance d(X,X_cf)/d(X,NUN)   (lower  = better)
"""
...
def compute_metrics(X_orig:   np.ndarray,
    ...
    Sigma_inv : (D, D) inverse covariance from training data
    X_train   : (N_tr, T, D)
    y_train   : (N_tr,)
    ...
    coherence = float(mean_diff @ Sigma_inv @ mean_diff)   # Mahalanobis

    cf_conf = float(probs[y_tgt])

    # Relative CF distance (RCF) = L2(X, X_cf) / L2(X, NUN)
    mask    = y_train != y_orig
...
def run_wilcoxon(tagfc_results:    list[dict],
    ...
    Bonferroni correction: alpha_corr = alpha / n_metrics.
    """
