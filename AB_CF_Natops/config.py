"""
config.py — Configuration for AB-CF on NATOPS dataset.
...
class Config:
    ...
    # ── AB-CF ──────────────────────────────────────────────────────────────────
    ABCF_N_SEGMENTS:   int   = 5       # number of segments to divide T into
    ABCF_N_BINS:       int   = 10      # bins for Shannon entropy histogram
    ABCF_WINDOW_FRAC:  float = 0.15    # minimum segment length as fraction of T
    ABCF_MAX_SWAPS:    int   = 50      # max (channel, segment) swaps before giving up
