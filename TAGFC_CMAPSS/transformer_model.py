"""
transformer_model.py — TransformerCMAPSS classifier with attention capture.

Architecture (pre-norm, same as TransformerNATOPS / TransformerAQI):
    (B, T=30, D=14)
      → Linear(14, 64) + sinusoidal PE
      → 3 × TransformerEncoderLayer (norm_first=True, batch_first=True)
      → LayerNorm → MeanPool → Dropout → Linear(64, 3)
"""
...
class TransformerCMAPSS(nn.Module):
    """3-class Transformer for C-MAPSS FD001 health-state classification."""
...
def load_model(path, c: Config = cfg) -> TransformerCMAPSS:
