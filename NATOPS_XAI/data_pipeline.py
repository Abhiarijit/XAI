"""
data_pipeline.py — NATOPS data loading and normalisation.
...
NATOPS .arff format:
    ...
    First 24*51 = 1224 comma-separated float values (24 channels × 51 timesteps,
    channel-major order: ch0_t0..ch0_t50, ch1_t0..ch1_t50, ...).
    Last value = class label (string "1".."6").
...
def _parse_arff(path: Path, T: int, D: int) -> tuple[np.ndarray, np.ndarray]:
    """
    Parse a UEA-format NATOPS .arff file.

    Format per data row:
        'ch0_t0,ch0_t1,...,ch0_tT\\nch1_t0,...,ch1_tT\\n...\\nch23_t0,...',class

    Each row is one quoted string containing D channel series separated by
    literal '\\n', followed by a comma and the float class label.
    """
    X_list, y_list = [], []
    in_data = False
    ...
    with open(path, "r") as f:
        for line in f:
            ...
            # Split off the class label: last comma-separated token after closing quote
            # Row looks like: '...',4.0
            last_comma = line.rfind(",")
            label_str  = line[last_comma + 1:].strip().strip("'\"")
            data_str   = line[:last_comma].strip().strip("'\"")

            # Split by literal \n to get D channel strings
            channel_strs = data_str.split("\\n")
            if len(channel_strs) != D:
                # Fallback: flat comma-separated (channel-major D*T values)
...
def load_natops(c: Config = cfg):
    ...
    Supports two loading paths:
      - .npy arrays (fast, created by download_natops.py via aeon/sktime)
      - .arff files (standard UEA format, parsed manually)
    ...
    if npy_train.exists() and npy_test.exists():
        ...
        if not train_path.exists() or not test_path.exists():
            raise FileNotFoundError(
                ...
                f"Run:  python download_natops.py  first."
            )
...
if __name__ == "__main__":
    ...
    print(f"Min/Max after normalisation: {X_tr.min():.3f} / {X_tr.max():.3f}")
