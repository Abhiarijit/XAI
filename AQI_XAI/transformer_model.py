"""
transformer_model.py — TransformerAQI classifier with attention capture.
...
Adapted from TransformerFD001 for AQI India: T=14, D=12, K=3.
"""
...
class TransformerAQI(nn.Module):
    """
    3-class Transformer classifier for AQI India (city_day.csv).
    ...
    def attention_rollout(self, x_np: np.ndarray) -> np.ndarray:
        """
        Abnar & Zuidema (2020) attention rollout saliency.
        x_np : (T, D) → returns s ∈ [0,1]^T
        """
        self.eval()
    ...
    def input_gradient(self, x_np: np.ndarray, target_class: int) -> np.ndarray:
        """Gradient of logit[target_class] w.r.t. input. x_np : (T, D) → (T, D)"""
        self.eval()
...
def load_model(path, c: Config = cfg) -> TransformerAQI:
    model = TransformerAQI(c)
